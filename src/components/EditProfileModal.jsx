...
export default function EditProfileModal({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. Upload avatar first if a new one was selected
      if (avatarFile) {
      ...
      <div className="bg-[#0d1117] border border-[#1f2937] rounded-2xl w-full max-w-md p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            >
              {avatarPreview ? (
                <img
                  src={avatarPreview}
                  alt="avatar"
                  className="w-20 h-20 rounded-full object-cover border-2 border-[#1f2937]"
                />
              ) : (
          ...
          <div>
            ...
            <div className="w-full bg-[#0d1117] border border-[#1f2937] text-gray-600 rounded-xl px-4 py-2.5 text-sm">
              @{user?.username}
            </div>
